...
class DummyEvent:
    Year: int = 2024
    RoundNumber: int = 9
    EventName: str = "Monaco Grand Prix"
    ...
    return pd.DataFrame(
        {
            ...
            "TyreLife": [1, 2, 1, 2],
            "Stint": [1, 1, 1, 1],
            "Team": ["Red Bull", "Red Bull", "McLaren", "McLaren"],
            "TrackStatus": ["1", "1", "1", "1"],
            "Position": [1, 1, 2, 2],
...
def test_build_canonical_laps_schema_types_and_non_empty() -> None:
    ...
    assert str(laps["lap_time_s"].dtype) == "float64"
...
def test_load_or_build_canonical_laps_rebuilds_outdated_cache(tmp_path: Path) -> None:
    session = DummySession(_fixture_laps())
    outdated_cache_path = tmp_path / "laps-2024-9-race-NOR-VER.parquet"
    pd.DataFrame({"driver": ["VER"]}).to_parquet(outdated_cache_path, index=False)

    rebuilt_df, loaded_from_cache, cache_path = load_or_build_canonical_laps(
        session,
        ["VER", "NOR"],
        cache_dir=tmp_path,
    )
    ...
    assert cache_path == outdated_cache_path
    assert list(rebuilt_df.columns) == REQUIRED_LAPS_COLUMNS
