...
def _render_driver_tab() -> None:
    ...
    exclude_sc_vsc = st.checkbox("Exclude SC/VSC laps from filtered views", value=False)
    ...
    if st.button("Build canonical laps table", type="primary"):
        ...
        try:
            ...
            try:
                ...
                st.session_state["exclude_sc_vsc"] = exclude_sc_vsc
...
def main() -> None:
    ...
    with tabs[6]:
        ...
        if dropped_laps is None:
            st.info("Build canonical laps to inspect filtering diagnostics.")
        elif dropped_laps.empty:
            st.info("No laps were dropped by the active filter rules.")
        else:
            st.write("Dropped laps")
            st.dataframe(dropped_laps, use_container_width=True)
